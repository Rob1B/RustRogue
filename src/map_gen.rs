use crate::{
    create_player,
    map::{new_map_rooms_and_corridors, TileType},
...
pub fn gen_map(height: u32, width: u32, ecs: &mut World) {
    let (rooms, map) = new_map_rooms_and_corridors(width, height);
    let map_entity = gen_map_from_tile(&map, ecs, width, height);
